...
\begin{remark}[Halley's Incomplete Explanation]
  Halley's theory attributed the trade winds purely to \textgr{differential solar heating}:
  the Sun heats the equatorial region more intensely than the poles, causing air to rise
  at the equator and drawing surface air equatorward.  While this correctly identifies
  the thermodynamic driver, his explanation suffers from several critical omissions:
  \begin{itemize}
    \item He failed to account for the \textgr{Earth's rotation} and the resulting
      Coriolis force, which is responsible for deflecting the meridional flow into
      the observed easterly direction.
    \item His theory cannot explain the \textgr{midlatitude westerlies}: without
      rotation, the circulation would consist of a single direct cell per hemisphere
      with surface winds directed uniformly from pole to equator.
    \item Without the Coriolis effect, there is no mechanism to produce the observed
      zonal (east--west) component of the wind at any latitude.
  \end{itemize}
