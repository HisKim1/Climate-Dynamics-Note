...
강화/수정한다.  Blackmon et al.\ (1977)이 twice-daily NMC operational
analyses (NCEP reanalysis 이전 시대) 자료를 시간 필터링하여 처음
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.65\textwidth,height=0.5\textheight,keepaspectratio]{images/ch06/p04_img01.png}
...
\hisu{Low-pass = 느린 신호만 통과 (low-frequency variability, $\approx 10$--$90$일);
High-pass = 빠른 신호 ($<2.5$일의 mesoscale);  Band-pass = 그 사이
(synoptic $\approx 2.5$--$6$일, baroclinic wave가 사는 대역).
Blackmon \emph{et al.}\ (1977)의 표준 구분이다.}
...
Blackmon et al.\ (1977)이 NMC operational 관측 자료에 적용한 방식을
그대로 50년 GCM pre-industrial control run의 일일 SLP에 적용하면 다음과
같은 분리가
가능하다.

\begin{itemize}
  \item \textbf{Low-pass} ($\approx 10$--$90$일 시간 스케일의 변동;
        low-frequency variability).  Stationary wave는 이 중에서도
        장기 시간 평균의 zonal anomaly로 따로 정의된다.
  \item \textbf{Band-pass} ($\approx 2.5$--$6$일): synoptic-scale baroclinic eddies.
        이 성분의 \textit{분산}이 storm track의 정의이다.
  \item \textbf{High-pass} ($<2.5$일): mesoscale 및 noise 성분.  일반적으로
        SLP에서는 작다.
\end{itemize}
\end{example}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.65\textwidth,height=0.5\textheight,keepaspectratio]{images/ch06/p05_img01.png}
...
물리적 의미가 명확하다.  Low-pass std는 ``느리게 변하는 기후 모드''
(\textgr{low-frequency variability}; PNA, NAO 등)의 흔적을 보여주고,
band-pass std는 \textit{storm track} 그 자체이다.  주의: \emph{stationary
wave}는 low-pass 분산이 아니라 \emph{장기 시간 평균} ($\overline{X}$)의
\emph{zonal anomaly} $X^* = \overline{X} - [\overline{X}]_\lambda$로 정의되는,
별개의 양임을 강조한다.  Low-pass variance는 단지 ``느린 변동''일 뿐이지
정상파 그 자체가 아니다.
\end{remark}

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.65\textwidth,height=0.5\textheight,keepaspectratio]{images/ch06/p07_img01.png}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.65\textwidth,height=0.5\textheight,keepaspectratio]{images/ch06/p08_img01.png}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.65\textwidth,height=0.5\textheight,keepaspectratio]{images/ch06/p09_img01.png}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.65\textwidth,height=0.5\textheight,keepaspectratio]{images/ch06/p11_img01.png}
  \caption{Idealized meridional cross-section of the midlatitude jet and its
  associated secondary circulation. \textbf{(a)} The pure thermal-wind jet
  alone: balanced flow has no surface wind and no eddy fluxes.
  \textbf{(b)} The observed climatology adds strong surface westerlies in the
  oceanic-low sector and strong convergence of eddy momentum flux at upper
  levels, together with strong eddy heat flux at lower levels.  These
  features are precisely what is needed to maintain the surface westerlies
  against friction and to balance the thermal-wind shear of the jet.
  \hisu{(a) thermal-wind jet 만으로는 표층 서풍·표층 마찰을 설명할 수 없다.
  (b) eddy momentum flux의 상층 수렴과 eddy heat flux의 하층 수송이
  서풍대를 유지한다.}}
...
% [Jet entrance/exit force-balance diagram: lecture slide had only a
%  simplified L/H hand sketch.  The 4-cell rising/sinking interpretation is
%  fully developed in the prose above and in Fig.~\ref{fig:ch06_nh_ageo_schem}'s
%  cross-section, so no separate diagram is needed here.]
...
\begin{proposition}[Lagrangian Acceleration in Jet Entrance/Exit]
$\beta y\,v_g$ 항이 $f_0 v_a$에 비해 작은 영역에서 \eqref{eq:ch06_nh_qg_u}는
\begin{equation}\label{eq:ch06_nh_qg_simple}
  \frac{d_g u_g}{dt} \;\approx\; f_0\, v_a
\end{equation}
로 단순화된다.  여기서 $d_g/dt = \partial/\partial t + \mathbf{v}_g\cdot\nabla$
는 geostrophic material derivative이며, 좌변은 입자의 \emph{Lagrangian}
가속도이다.
\end{proposition}

\begin{proof}
\eqref{eq:ch06_nh_qg_u}을 정의대로
$d_g u_g/dt - f_0 v_a - \beta y v_g = 0$.
Jet entrance 부근에서는 입자가 jet core를 향해 흐르면서 가속되는
($d_g u_g/dt > 0$) 효과가 가장 크고, $\beta y v_g$ 항은 부차적이므로
...
\begin{equation}\label{eq:ch06_nh_va_estimate}
  v_a \approx \frac{1}{f_0}\frac{d_g u_g}{dt}
...
$d_g v_g/dt \approx -f_0 u_a$를 얻는다.  Continuity equation
...
\textit{시간 평균} ($\partial u_g/\partial t = 0$, advection도 시간 평균) 상태에서
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.65\textwidth,height=0.5\textheight,keepaspectratio]{images/ch06/p17_img01.png}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.65\textwidth,height=0.5\textheight,keepaspectratio]{images/ch06/p18_img01.png}
  \caption{Composite structure of synoptic-scale transient vertical heat
  flux $-\overline{\omega' T'}$ in the NH winter, after Lau (1979).
  \textbf{(top)} $-\overline{\omega' z'}$ at $500\,\text{hPa}$ and
  \textbf{(middle)} $300\,\text{hPa}$ — vertical fluxes of geopotential
  expressing storm-track amplitude; \textbf{(bottom)}
  $-\overline{\omega' T'}$ at $700\,\text{hPa}$ — vertical heat flux
  showing the baroclinic conversion footprint over the western Pacific
  and western Atlantic storm tracks.  The lower-tropospheric heat flux
  shows the largest magnitude (positive: warm air rising, cold air
  sinking), confirming the baroclinic growth mechanism.
  \hisu{Lau (1979a)의 transient eddy 연직 수송장.  500/300\,hPa의 $-\overline{\omega' z'}$
  (위·중간)과 700\,hPa의 $-\overline{\omega' T'}$ (아래).  하층 $-\overline{\omega' T'}>0$
  최댓값이 storm track 위에서 \emph{baroclinic conversion}을 직접 나타낸다.}}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.75\textwidth,height=0.4\textheight,keepaspectratio]{images/ch06/p19a_img01.png}
  \caption{Vertical cross-section (longitude--pressure) of a \emph{developing}
  baroclinic disturbance.  Solid lines:
  geopotential height anomalies (trough/ridge); dashed lines: temperature
  anomalies (WARM, COLD); arrows:
...
$\partial \overline{u}/\partial p < 0$ (westerly shear with height), a
westward phase tilt with height of the geopotential perturbation $\Phi'$
\emph{favors positive baroclinic energy conversion} from the mean available
potential energy to the eddy.  Equivalently, the upper-level pressure
trough lying upstream (west) of the lower-level trough is the
\emph{necessary structural signature} of poleward eddy heat flux,
\[
  \text{westward phase tilt with height}
  \;\Longrightarrow\;
  \overline{v'T'} > 0 .
\]
Strict equivalence requires additional assumptions on horizontal phase
structure and meridional temperature gradient sign, so the relation should
be read as ``necessary but not strictly sufficient.''
\end{theorem}
...
(ii) 그 줄어든 mean APE가 \emph{eddy APE}로 옮겨 가며,
다시 eddy APE가 $K_T$로 변환된다.
즉 \textbf{서쪽 기울기}는 \emph{baroclinic growth의 강력한 시각적 표지}
이지만, 엄밀한 ``iff''가 아니라 \emph{필요조건에 가까운 구조적 신호}로
이해해야 한다.
\end{hisublock}

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.75\textwidth,height=0.4\textheight,keepaspectratio]{images/ch06/p19b_img01.png}
  \caption{Same series as Fig.~\ref{fig:ch06_te_developing_xsec} but for the
  \emph{decaying} (occluded) stage.  The vertical tilt has largely
  disappeared: upper and surface troughs are nearly co-located, warm and
  cold anomalies have wrapped around the surface low, and the $\omega$
  couplet has weakened.  \hisu{쇠퇴기에는 상하층 trough가 거의 일직선상에 정렬되어
...
during the growth phase the westward tilt drives the cascade
\emph{mean APE} $\to$ \emph{eddy APE} $\to$ \emph{$K_T$} ---
westward tilt is the structural signature of the first
($\overline{P}_M \to P_T$) conversion, while $-\overline{\omega'\alpha'}>0$
(warm rising, cold sinking) closes the second ($P_T \to K_T$) conversion;
the wave saturates as the tilt collapses, and ultimately deposits its
kinetic energy back into the zonal flow at upper levels
(\emph{barotropic decay}).
\end{remark}
...
Lorenz (1955) energy-cycle 표준 표기에 따라
\begin{align}
  \frac{\partial \overline{K_M}}{\partial t}
    &= -\nabla\!\cdot\!\bigl(H_0 + H_1\bigr)
       - \frac{\partial}{\partial p}\!\left(\overline{K_M}\,\overline{\omega}\right)
       \nonumber\\
    &\quad
...
\end{align}
where the four horizontal flux terms are defined (Lorenz 1955,
Peixoto \& Oort 1992 Ch.~14 표기) as
\begin{align}
  H_0 &= \overline{\mathbf{V}}\;\overline{K_M},
    \label{eq:ch06_te_H0}\\
  H_1 &= \overline{\mathbf{V}'\Phi'}
       \quad(\text{eddy geopotential flux}),
    \label{eq:ch06_te_H1}\\
  ...
  H_3 &= \tfrac{1}{2}\,\overline{(u'^2 + v'^2)\,v'},
    \label{eq:ch06_te_H3}
\end{align}
with $K_T' \equiv \tfrac{1}{2}(u'^2 + v'^2) - K_T$ the instantaneous fluctuation
of eddy KE about its time-mean $K_T$.  The cross-term $C(\overline{K_M},K_T)$
is the \textgr{barotropic conversion} between mean and eddy KE.
\end{theorem}
...
\begin{itemize}
  ...
  \item $H_1 = \overline{\mathbf{V}'\Phi'}$ (eddy geopotential flux):
        Lorenz cycle의 \emph{ageostrophic pressure work} 항.
        Eddy가 평균 흐름에 ``일''을 해주는 통로이며, 본 항이
        $-\overline{\mathbf{V}'\cdot\nabla\Phi'}$와 쌍을 이룬다.
  \item $H_2 = \overline{\mathbf{V}'\,K_T'}$:
        eddy 자체가 \emph{자기 KE 요동}을 자오선 방향으로 운반.
        ($K_T' = \tfrac{1}{2}(u'^2+v'^2) - K_T$.
        만일 단순히 $\overline{\mathbf{V}'}\,K_T$로 쓰면 $\overline{\mathbf{V}'}=0$
        이라 항이 사라지므로, 반드시 \emph{요동의 product}로 표기해야 한다.)
  \item $H_3 = \tfrac{1}{2}\,\overline{(u'^2+v'^2)v'}$:
        eddy가 \emph{자기 자신의} KE를 자오선 방향으로 self-transport.
        storm track에서 가장 중요한 항.
\end{itemize}
이 분해는 Lau (1979a)의 Eqs.~(2)--(3)와 Peixoto \& Oort (1992) Ch.~14의
표기에 대응한다.
\end{hisublock}
...
위의 KE budget을 \textgr{Lorenz (1955)의 고전적 atmospheric energy cycle}
도식으로 정리하면 다음과 같다.  네 개의 \emph{reservoir}와 그 사이의
주요 흐름을 화살표로 표시한다:
\[
\begin{array}{ccccc}
  \overline{P_M} & \xrightarrow{C(\overline{P_M},P_T)} & P_T & & \\
  {\scriptstyle G_M\uparrow} & & {\scriptstyle\downarrow C(P_T,K_T)} & & \\
  & & K_T & \xrightarrow{C(\overline{K_M},K_T)} & \overline{K_M} \\
  & & {\scriptstyle\downarrow D_T} & & {\scriptstyle\downarrow D_M}
\end{array}
\]
즉 네 reservoir는
\textbf{mean APE} $\overline{P_M}$,
\textbf{transient APE} $P_T$,
\textbf{transient KE} $K_T$,
\textbf{mean KE} $\overline{K_M}$이며,
화살표의 양의 방향이 baroclinic instability에 의한 ``정상'' eddy 성장
($\overline{v'T'}>0$, $-\overline{\omega'\alpha'}>0$)을 나타낸다.
$\overline{K_M}\to K_T$ 방향의 conversion이 음이면 (즉 $K_T\to\overline{K_M}$)
이는 ``barotropic decay'' 분기에 해당한다.

\medskip

\begin{remarkbox}
\textbf{Lorenz cycle의 핵심 발견:}
\begin{enumerate}
  \item Diabatic forcing $G_M$이 적도-극 온도 경사 형태의 $\overline{P_M}$을 생성.
  \item Baroclinic instability $C(\overline{P_M}, P_T)$가 $\overline{v'T'}>0$를 통해 $\overline{P_M}\to P_T$ 전달.
  \item Adiabatic conversion $C(P_T, K_T)$가 $-\overline{\omega'\alpha'}>0$를 통해 transient eddy KE를 공급.
  \item Barotropic conversion $C(\overline{K_M}, K_T)$가 mean KE와 eddy KE 사이의 \emph{보통 음수} 흐름 (즉 eddy → mean) — Reynolds stress의 \textit{upgradient} 성격.
  \item Frictional dissipation $D_M, D_T$가 최종적으로 KE를 제거하여 cycle을 닫는다.
\end{enumerate}
\end{remarkbox}
...
\begin{itemize}
  ...
  \item $C(P_T,K_T)$: $-\overline{\omega'\alpha'}>0$
        (등가식 $-(R/p)\overline{\omega'T'}>0$;
        압력 좌표에서 \emph{warm} $T'>0$ 공기가 \emph{상승} $\omega'<0$하므로
        $\omega'T'<0$이고 따라서 $-\overline{\omega'T'}>0$)에 의한
        $P_T\to K_T$ 변환.
...
\begin{enumerate}
  ...
  \item \textbf{KE budget}: $\overline{K_M}$과 $K_T$의 진화 방정식은
        ...
        instability.
  \item \textbf{에너지 cycle}: Lorenz 4-box reservoir 모형으로 전체 cycle을
        통합한다 (§2.4의 도식과 \textit{Lorenz cycle의 핵심 발견} remarkbox 참조).
...
그러나 \emph{시간 평균} 운동량 플럭스 $\overline{u'v'}$, 열 플럭스 $\overline{v'T'}$,
...
\begin{equation}\label{eq:ch06_sw_vorbal_final}
  ...
  \;=\; (f + \overline{\zeta}_g)\,\frac{\partial \overline{\omega}}{\partial p}
  \;}
\end{equation}
이 된다.  우변은 평균 발산에 의한 \textgr{vortex stretching}이며,
continuity equation $\nabla\!\cdot\!\overline{\mathbf{V}} = -\partial\overline{\omega}/\partial p$
를 이용하면 등가적으로 $(f+\overline{\zeta}_g)\,\partial\overline{\omega}/\partial p$
형태로 표현된다.
좌변은 (1) 평균 흐름에 의한 평균 vorticity 이류, (2) beta 효과,
(3) transient eddy의 vorticity flux convergence 세 항이다.
...
강제 상승/하강이다.  먼저 geometric vertical velocity $w_M = \overline{\mathbf{V}}\!\cdot\!\nabla h$
를 정의하고, $\omega \approx -\rho g\, w$ (hydrostatic, $Dp/Dt \approx -\rho g \,Dz/Dt$)를 이용해
압력 좌표로 변환하면
\begin{equation}\label{eq:ch06_sw_omegaM}
  \overline{\omega}_M \;=\; -\,\rho g\;\overline{\mathbf{V}}\!\cdot\!\nabla h ,
\end{equation}
여기서 $h(\lambda,\phi)$는 지표면 고도, $\rho$는 지표면 부근 공기 밀도다.
단위는 $\mathrm{Pa\,s^{-1}}$로, $\omega$의 표준 단위와 일치한다.

\textgr{Frictional (Ekman)} 성분은 마찰 경계층 안에서 발생하는 Ekman pumping/suction이다.
우선 free-atmosphere 직하에서의 geometric Ekman 속도를
$w_E = \sqrt{K_m/(2f)}\;\overline{\zeta}_g$로 두고(``Ekman pumping''의 표준 형태,
Holton \& Hakim Ch.~5), 압력 좌표로 변환하면
\begin{equation}\label{eq:ch06_sw_omegaF}
  \overline{\omega}_F \;=\; -\,\rho g\, w_E
                       \;=\; -\,\rho g\,\sqrt{\frac{K_m}{2 f}}\;\overline{\zeta}_g ,
\end{equation}
여기서 $K_m$은 경계층 eddy viscosity다 (cyclonic $\overline{\zeta}_g>0$에서
$w_E>0$ 상승 $\Leftrightarrow$ $\overline{\omega}_F<0$, Pa/s 단위).
\end{definition}
...
$\overline{\omega}_M = -\rho g\,\overline{\mathbf{V}}\!\cdot\!\nabla h < 0$,
즉 \textgr{forced ascent}; 동측 사면에서는 $\overline{\omega}_M > 0$, 즉
forced descent가 일어난다.
이 \textbf{mountain torque}가 stationary wave의 가장 강력한 sources 중 하나다.
Friction 항 \eqref{eq:ch06_sw_omegaF}은 cyclonic vorticity 영역에서
$\overline{\omega}_F<0$ (ascent), anticyclonic 영역에서 $\overline{\omega}_F>0$
(descent)를 일으켜 평균 vorticity의 \emph{spin-down}에 기여한다.
\end{remark}
...
\begin{equation}\label{eq:ch06_sw_heatfull}
  ...
  \;+\; \Bigl[\,\frac{\partial}{\partial p}\overline{\omega'T'}
              \;-\; \frac{R}{c_p\,p}\,\overline{\omega'T'}\,\Bigr]
  \;=\; \frac{\overline{\dot{Q}}}{c_p}
  \;}
\end{equation}
여기서 각 항의 물리적 의미는:
\begin{itemize}
  ...
  \item 사각 괄호 항: transient eddy vertical heat 수송 (보통 작음).
        $1/p$ 인자는 압력 좌표에서 단열 항의 정확한 형태를 보장한다
        (Holton \& Hakim Eq.~6.33 표기).
  \item $\overline{\dot{Q}}/c_p$: \textgr{diabatic heating} per unit mass
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.65\textwidth,height=0.5\textheight,keepaspectratio]{images/ch06/p35_img01.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.65\textwidth,height=0.5\textheight,keepaspectratio]{images/ch06/p36_img01.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.65\textwidth,height=0.5\textheight,keepaspectratio]{images/ch06/p37_img01.png}
...
\noindent\textbf{Mean vorticity advection: rotational vs.\ planetary contributions.}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.65\textwidth,height=0.5\textheight,keepaspectratio]{images/ch06/p38_img01.png}
  \caption{Time-mean horizontal advection of absolute vorticity at 300\,hPa
    for DJF, decomposed into three components.  \textbf{(a)} Total advection
    $-\overline{\mathbf{V}}\cdot\nabla(\overline{\zeta}+f)$;
    \textbf{(b)} relative-vorticity contribution
    $-\overline{\mathbf{V}}\cdot\nabla\overline{\zeta}$;
    \textbf{(c)} planetary-vorticity (beta) contribution
    $-\overline{V}\,df/dy = -\beta \overline{V}$.
    The total field (a) is dominated by the beta term (c) in midlatitudes,
    while the relative-vorticity term (b) is locally important over the
    storm-track exit regions.
    \hisu{상층 평균 vorticity 이류장 세 성분.  (a) 전체, (b) 상대 vorticity
          이류, (c) 행성 vorticity 이류 ($\beta\overline{V}$).
          중위도 평균은 $\beta\overline{V}$가 지배.}}
  \label{fig:ch06_sw_momflux}
\end{figure}

그림 \ref{fig:ch06_sw_momflux}는 §3의 vorticity balance
\eqref{eq:ch06_sw_vorbal_final}의 좌변(평균 vorticity 이류)을 관측적으로 보여준다.
중위도에서는 $\beta\overline{V}$ 항이 가장 크고, 부호는 ridge 동쪽($\overline{V}>0$)에서 음
($-\beta\overline{V}<0$), trough 동쪽에서 양으로 분리된다.
이 이류가 어떻게 평균 상태에서 균형을 이루는지는 다음 그림의 eddy 항이 보여준다.

\medskip



\noindent\textbf{Eddy vorticity flux convergence and horizontal diffusion.}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.65\textwidth,height=0.5\textheight,keepaspectratio]{images/ch06/p39_img01.png}
  \caption{Time-mean fields at 300\,hPa for DJF that close the upper-level
    vorticity balance.  \textbf{(top)} Convergence of transient eddy
    vorticity flux, $-\nabla\!\cdot\!\overline{\mathbf{v}'\zeta'}$;
    \textbf{(bottom)} parameterised horizontal diffusion of mean vorticity,
    $\overline{D}_H$.  Together these provide the dominant sinks that
    balance the mean advection of Fig.~\ref{fig:ch06_sw_momflux}.
    \hisu{상층 vorticity 균형의 우변: (위) eddy vorticity flux 수렴
          $-\nabla\!\cdot\!\overline{\mathbf{v}'\zeta'}$,
          (아래) horizontal diffusion $\overline{D}_H$.
          평균 이류를 상쇄해 stationary wave 유지.}}
  \label{fig:ch06_sw_heatflux1000}
\end{figure}

상층 ($300$\,hPa) vorticity balance는 평균 이류
(Fig.~\ref{fig:ch06_sw_momflux}a)와 eddy flux 수렴
$-\nabla\!\cdot\!\overline{\mathbf{v}'\zeta'}$가 \emph{서로 거의 같은 크기이지만
부호가 반대}임을 보여준다.  즉 transient eddy의 vorticity 수송이 평균 이류를
정확히 상쇄하여 stationary wave가 정상상태로 유지된다.  Horizontal diffusion
$\overline{D}_H$은 보조적 sink로서 잔여 불균형을 마무리한다.

\medskip



\noindent\textbf{Low-level heat budget: mean advection vs.\ eddy flux convergence.}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.65\textwidth,height=0.5\textheight,keepaspectratio]{images/ch06/p40_img01.png}
  \caption{Time-mean heat budget terms at 1000\,hPa for DJF.
    \textbf{(top)} Horizontal advection of mean temperature
    $-\overline{\mathbf{V}}\cdot\nabla\overline{T}$;
    \textbf{(bottom)} convergence of transient eddy heat flux
    $-\nabla\!\cdot\!\overline{\mathbf{v}'T'}$.
    The two fields are nearly mirror images: storm-track regions show
    eddy heat-flux convergence that warms the lower troposphere, partially
    cancelled by mean cold-advection on the western side of continental
    cold lows.  This pairing realises the heat-budget identity
    \eqref{eq:ch06_sw_heatfull} term by term.
    \hisu{하층 ($1000$\,hPa) 평균 heat budget의 두 항.  (위) 평균 온도이류,
          (아래) eddy heat flux 수렴.  두 항이 거의 정반대 부호로 짝을 이뤄
          \eqref{eq:ch06_sw_heatfull} 균형을 만든다.}}
  \label{fig:ch06_sw_composites}
\end{figure}

이 그림은 본 절의 \emph{최종 종합도}로 읽혀야 한다.  평균 온도이류와 eddy
heat flux 수렴이 거의 정반대 부호로 \emph{공간적으로 짝을 이루는} 모습이
quarter-phase shift theorem \eqref{eq:ch06_sw_qps}의 관측적 검증을 제공한다.
하층 heat budget의 잔여 불균형은 다음 그림의 diabatic heating으로 흡수된다.

\medskip



\noindent\textbf{Low-level diabatic heating.}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.65\textwidth,height=0.5\textheight,keepaspectratio]{images/ch06/p41_img01.png}
  \caption{Time-mean diabatic heating $\overline{Q}$ at 1000\,hPa for DJF,
    diagnosed as the residual of the observed heat budget
    \eqref{eq:ch06_sw_heatfull}.  Positive heating maxima sit over the
    western boundary currents (Kuroshio, Gulf Stream) and the warm-pool
    region, reflecting sensible/latent heat release into the marine
    boundary layer; negative values over winter continents reflect
    long-wave cooling and snow-cover albedo.  $\overline{Q}$ is what
    \emph{drives} the entire stationary wave: it is the inhomogeneous
    forcing on the right-hand side of \eqref{eq:ch06_sw_heatfull}.
    \hisu{지표 평균 diabatic heating $\overline{Q}$.  Kuroshio·Gulf Stream
          위에서 가열, 겨울 대륙에서 냉각.  이 비등방 forcing이 stationary
          wave 전체를 ``돌리는'' 근원.}}
...
지표면의 비등방 diabatic heating $\overline{Q}$가
\eqref{eq:ch06_sw_heatfull}의 우변 forcing으로 작용하여 stationary $T_E$를 만들고,
이 $T_E$는 hypsometric 관계를 통해 $Z_E$ 구조를 결정한다.
$Z_E$로부터 $\overline{\omega}$가 \eqref{eq:ch06_sw_omegaF}--\eqref{eq:ch06_sw_omega_integral}을
통해 결정되고, 다시 vorticity stretching $(f+\overline{\zeta})\nabla\!\cdot\!\overline{\mathbf{V}}$를
통해 \eqref{eq:ch06_sw_vorbal_final}의 vorticity 균형을 닫는다.
즉 \textbf{stationary wave는 지표 diabatic forcing → 온도장 → 고도장 →
연직 운동 → vorticity 균형으로 이어지는 닫힌 회로}이다.
\end{remark}

\hisu{지표 diabatic heating → 온도 이상 $T_E$ → 고도 이상 $Z_E$ → 연직 운동
$\overline{\omega}$ → vorticity stretching → 다시 vorticity 균형.
이 닫힌 회로가 stationary wave 유지의 본질.}
...
\begin{equation}\label{eq:ch06_dd_held_qg}
  ...
  \;+\; \beta\,\pdx{\psi'} \;=\; F(x,y),
\end{equation}
여기서 $F$는 zonal flow $\bar u$가 산악을 가로지를 때의 vorticity stretching에서 비롯된 강제
\begin{equation}\label{eq:ch06_dd_held_topoforcing}
  F_{\text{topo}}(x,y) \;=\; -\,\frac{f_0\,\bar u}{H}\,\pdx{h_{\text{topo}}} ,
\end{equation}
또는 diabatic vorticity source이다 (1D zonal flow over orography에서의 표준 형태,
Held 1983 Eq.~2.4).
...
\end{equation}
의 \textgr{stationary total wavenumber} $K_s^2 = \beta/\bar u$를 정의한다.
지형 강제에 대한 정상상태 응답은
\begin{equation}\label{eq:ch06_dd_held_topo_response}
  \hat\psi_{\text{topo}}
  \;\propto\; \frac{f_0 h_0 / H}{(k^2 + l^2) - K_s^2},
\end{equation}
이므로 $(k^2+l^2) \to K_s^2$에서 응답이 발산(resonance)한다.  Held는 베타 평면 분석에 이어
구면(sphere) 모델로 확장하고, 실제 지형과 diabatic forcing을 함께 부과해 응답 패턴을 그린다.

\medskip
\noindent\textbf{Key Results.}
\begin{itemize}
  \item Mean wind $\bar u$가 약해질수록 $K_s = \sqrt{\beta/\bar u}$는 \emph{커지고}
        (즉 정상파 wavelength는 짧아지며), 주어진 zonal wavenumber $k$가 더 작은
        $K_s$로부터 멀어지거나 가까워지는지에 따라 응답 진폭이 결정된다.
        실제 NH 겨울 jet의 약화는 일반적으로 stationary response 진폭을 키우는
        방향으로 작용한다 (Held 1983 Fig.~6.4).
...
따라 서편(westward tilt)으로 기울어 있어 압력 좌표에서 \emph{warm} ($T'>0$) 공기가
\emph{상승} ($\omega'<0$)함에 따라 $\omega'T'<0$, 즉 $-\overline{\omega'T'}>0$
(baroclinic conversion이 양)이 되는지 --- 는 정량 검증이 필요했다.
Lau (1979a)는 다층 자료에 KE 수지를 적용해 이 질문에 답한다.
...
\textbf{Lau, N.-C., 1979: The observed structure of tropospheric stationary waves and the local balances of vorticity and heat.
\emph{J.\ Atmos.\ Sci.}, \textbf{36}, 996--1016.  [Lau~1979b]}
...
\textbf{Chen, T.-C., Yoon, J.-H., and Wang, S.-Y., 2005:
Westward propagation of the Indian monsoon depression.
\emph{Tellus~A}, \textbf{57}, 758--769.}
...
\begin{enumerate}
  ...
  \item \textgr{Stationary waves}는 두 메커니즘으로 강제된다:
        ...
        $\hat\psi \propto h_0/[(k^2+l^2) - K_s^2]$에서 $K_s = \sqrt{\beta/\bar u}$
...
본 챕터에서 ``transient/stationary $\overline{u'v'}$가 jet을 유지한다''는
...
Lau (1979b), \emph{J.~Atmos.~Sci.}, \textbf{36}, 996--1016;
Chen, Yoon, and Wang (2005), \emph{Tellus~A}, \textbf{57}, 758--769.}
