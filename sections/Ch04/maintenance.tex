...
If air in the upper branch of the Hadley cell conserved angular momentum perfectly as it moves poleward from the equator, we would predict:
$$u(\phi) = \Omega a\left(\frac{\cos^2\phi_0}{\cos\phi} - \cos\phi\right)$$
For $\phi_0 = 0^\circ$ (equatorial origin) and $\phi = 30^\circ$, this gives $u \approx 134$\,m\,s$^{-1}$ --- far exceeding the observed subtropical jet of $\sim$30--40\,m\,s$^{-1}$. This discrepancy demonstrates that \textgr{eddies} must be extracting angular momentum from the subtropics and depositing it in the midlatitudes, decelerating the subtropical jet.
\end{remark}
\begin{hisublock}
...
The position and strength of the subtropical and eddy-driven jets depend on the interaction between the thermally driven Hadley cell and midlatitude eddies:
\begin{itemize}
\item \textgr{Tropical warming} increases the meridional temperature gradient at the poleward edge of the Hadley cell, strengthening baroclinicity and shifting the storm track poleward.
\item The Hadley cell edge (where the thermally direct circulation transitions to eddy-driven dynamics) moves poleward under warming, taking the subtropical jet with it.
\item This poleward shift of baroclinic activity further shifts the eddy-driven jet, amplifying the response.
\end{itemize}
\end{remark}
\begin{hisublock}
...
\noindent\textbf{First term: planetary vorticity.}\quad
...
\noindent\textbf{Second term: vorticity flux $=$ convergence of momentum flux.}
...
\noindent\textbf{Third term: thickness flux $=$ vertical derivative of heat flux.}
...
This principle explains how \textgr{self-sustaining eddy-driven jets} work:
\begin{enumerate}
\item Baroclinic eddies generated in the lower troposphere propagate upward and equatorward.
\item Their EP flux convergence in the upper troposphere drives a residual circulation via downward control.
\item This residual circulation maintains the surface westerlies against friction, sustaining the very baroclinicity that generates the eddies --- a self-sustaining feedback loop.
\end{enumerate}
