...
\begin{figure}[ht]
  ...
  \includegraphics[width=0.7\textwidth]{images/ch03/p09_img01.png}
...
\begin{figure}[ht]
  ...
  \includegraphics[width=0.7\textwidth]{images/ch03/p14_img01.jpeg}
...
\begin{figure}[ht]
  ...
  \includegraphics[width=0.7\textwidth]{images/ch03/p13_img02.jpeg}
...
\begin{hisublock}
\textbf{$\langle\omega T\rangle_{ZM}$이 작은 이유.}
\eqref{eq:ch03_hw4_zvT_final}의 단열 항 $-\langle R\omega T/(c_{p}p)\rangle_{ZM}$이 실제 관측에서 작은 잔차로
나오는 것은 두 가지 이유에 기인한다.  (i) $\omega$ 자체가 zonal mean에서 한 자릿수 작다 ---
Fig.~\ref{fig:ch03-zmvw}에서 본 $[\bar v]$가 $\sim$0.3\,m\,s$^{-1}$로 매우 작은 것과 같은 맥락이며,
\textbf{자오 순환(Hadley/Ferrel cell)의 vertical heat flux가 약하다}는 사실의 정량적 표현이다.
(ii) $T$는 연직으로 단조 감소하므로 $\langle T\rangle_{ZM}$ 부근에서 $\omega'T'$ 상관이 거의 상쇄된다.
결국 \eqref{eq:ch03_hw4_zvT_final}에서 \emph{지배항은 첫째 항(meridional eddy heat flux divergence)과
우변(다이아베틱 가열)} 두 개뿐이며, 이것이 \S\ref{sec:ch03-eddy-decomp}의
$\overline{v_{E}^{2}}$ 구조 \eqref{eq:vE2-structure}가 climate diagnostics에서 중요한 이유이다 ---
$\overline{v_{E}^{2}}$가 큰 위도가 곧 $\langle vT\rangle_{ZM}$이 클 가능성이 높은 위도이다.
\end{hisublock}
...
\begin{figure}[ht]
  ...
  \includegraphics[width=0.7\textwidth]{images/ch03/p32_img01.png}
