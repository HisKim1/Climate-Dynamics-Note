...
The zonally asymmetric component of the circulation (the stationary waves) arises from:
\begin{itemize}
\item \textgr{Orographic forcing}: flow over major mountain ranges (Rockies, Himalayas/Tibet)
\item \textgr{Thermal forcing}: land--sea heating contrasts and SST gradients
\end{itemize}

\begin{remark}[Cross-Equatorial Stationary Wave Response]
Despite asymmetric thermal forcing confined to one hemisphere during solstitial seasons, the stationary wave response can be nearly symmetric about the equator. Two mechanisms explain this:
\begin{enumerate}
\item \textgr{Rossby wave source}: upper-level divergence from tropical heating generates Rossby wave sources that propagate into both hemispheres via the waveguide effect.
\item \textgr{Cross-equatorial mean flow}: the strong upper-level cross-equatorial flow in the Hadley cell acts as a waveguide, transmitting wave activity across the equator (Hoskins and Karoly, 1981).
\end{enumerate}
\end{remark}
\begin{hisublock}
...
This is a direct consequence of the thermal wind relation~\eqref{eq:thermal-wind-ch03}: the
...
\begin{remark}[Why JJA Cross-Equatorial Flow is Stronger than DJF]
The cross-equatorial Hadley overturning is stronger in JJA than DJF because:
\begin{enumerate}
\item The Asian summer monsoon provides a massive additional heat source in the NH subtropics, intensifying the cross-equatorial pressure gradient.
\item The thermal equator shifts farther north in JJA ($\sim$20$^\circ$N over Asia) than it shifts south in DJF ($\sim$10$^\circ$S), creating a more asymmetric forcing.
\item Land--sea distribution: the NH has more land mass, amplifying the seasonal heating contrast.
\end{enumerate}
\end{remark}
\begin{hisublock}
