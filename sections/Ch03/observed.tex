...
\begin{figure}[ht]
  ...
  \includegraphics[width=0.7\textwidth]{images/ch03/p03_img01.png}
...
\begin{figure}[ht]
  ...
  \includegraphics[width=0.7\textwidth]{images/ch03/p05_img01.png}
...
\begin{figure}[ht]
  ...
  \includegraphics[width=0.7\textwidth]{images/ch03/p06_img01.jpeg}
...
\begin{figure}[ht]
  ...
  \includegraphics[width=0.7\textwidth]{images/ch03/p04_img01.png}
...
\begin{remark}[Major Pressure Centres]
...
대기 변수를 위도 $\phi$, 경도 $\lambda$, 연직좌표 $p$로 기술하기 전에,
구면좌표 자체의 미분연산자 형태를 한 번 정리해 두면 zonal mean 방정식 유도가 훨씬 깔끔해진다.

\begin{example}[구면좌표에서의 $\nabla f$와 $\nabla^{2}f$]
\label{ex:ch03_hw4_spherical}
구면좌표 $(r, \phi, \lambda)$ ($\phi$: 위도, $\lambda$: 경도) 위의 임의 스칼라 $f(r,\phi,\lambda)$를 생각하자.
$x = r\cos\phi\cos\lambda$, $y = r\cos\phi\sin\lambda$, $z = r\sin\phi$로부터
직교좌표 단위벡터를 구면좌표 단위벡터 $(\hat e_{r},\hat e_{\phi},\hat e_{\lambda})$로 표현하고
연쇄법칙을 적용하면, 한 단계 유도로
\begin{equation}
  \nabla f \;=\; \hat e_{r}\,\frac{\partial f}{\partial r}
            \;+\; \hat e_{\phi}\,\frac{1}{r}\,\frac{\partial f}{\partial \phi}
            \;+\; \hat e_{\lambda}\,\frac{1}{r\cos\phi}\,\frac{\partial f}{\partial \lambda},
  \label{eq:ch03_hw4_gradf}
\end{equation}
가 얻어진다.  여기서 $\lambda$ 방향 미분에 등장하는 $1/(r\cos\phi)$ 인자는
``경도선이 극으로 갈수록 좁아진다''는 측도 보정이다.  이어서
$\nabla^{2}f = \nabla\cdot(\nabla f)$를 같은 단위벡터로 풀어쓰면
\begin{equation}
  \nabla^{2} f \;=\; \frac{1}{r^{2}}\frac{\partial}{\partial r}\!\left(r^{2}\frac{\partial f}{\partial r}\right)
                 + \frac{1}{r^{2}\cos\phi}\frac{\partial}{\partial \phi}\!\left(\cos\phi\,\frac{\partial f}{\partial \phi}\right)
                 + \frac{1}{r^{2}\cos^{2}\phi}\,\frac{\partial^{2} f}{\partial \lambda^{2}}.
  \label{eq:ch03_hw4_lapf}
\end{equation}
대기과학에서는 $r \approx a$ (지구 반지름) 근사로 $\nabla \to \nabla_{H}$ (수평) 부분만 사용하는 일이 많으며,
이때 발산 연산자는
\begin{equation}
  \nabla_{H}\!\cdot\vec A \;=\; \frac{1}{a\cos\phi}\,\frac{\partial}{\partial\phi}\!\bigl(A_{\phi}\cos\phi\bigr)
                              + \frac{1}{a\cos\phi}\,\frac{\partial A_{\lambda}}{\partial\lambda}
  \label{eq:ch03_hw4_divH}
\end{equation}
이다.
\end{example}

\begin{hisublock}
이 결과 \eqref{eq:ch03_hw4_gradf}--\eqref{eq:ch03_hw4_divH}는 본 \S\ref{sec:ch03-eddy-decomp} 이하의
zonal-mean 식 유도(특히 \eqref{eq:ch03_hw4_zvT_main}의 $\partial_{\phi}(\langle vT\rangle_{Z}\cos\phi)$ 항)와
\S\ref{sec:ch03-thermal-wind}의 thermal wind 유도에서 그대로 사용된다.
$\cos\phi$ 가중치가 도처에 나타나는 것은 ``meridional flux는 면적요소 $a\cos\phi\,d\phi$ 위에서 정의된다''는
구면좌표의 본질을 반영한다.
\end{hisublock}
...
\begin{proposition}[Zonal-mean operator의 핵심 성질]
\label{prop:ch03_hw4_zonalvanish}
경도 $\lambda$에 대해 $2\pi$-주기적인 임의의 스무드 장 $a(\lambda,\phi,p,t)$에 대해
$\langle a\rangle_{Z} \equiv \frac{1}{2\pi}\int_{0}^{2\pi} a\,d\lambda$로 zonal mean을 정의하면
\begin{equation}
  \bigl\langle \partial_{\lambda} a \bigr\rangle_{Z} \;=\; 0.
  \label{eq:ch03_hw4_zonalvanish}
\end{equation}
\end{proposition}

\begin{proof}
정의에 따라
$\bigl\langle \partial_{\lambda} a\bigr\rangle_{Z}
= \frac{1}{2\pi}\int_{0}^{2\pi}\frac{\partial a}{\partial \lambda}\,d\lambda
= \frac{1}{2\pi}\bigl[a(\lambda,\phi,p,t)\bigr]_{0}^{2\pi}$.
$a$가 $\lambda$에 대해 $2\pi$-주기이므로 $a(2\pi,\cdot) = a(0,\cdot)$이고, 따라서 우변은 $0$이다.
\end{proof}

\begin{hisublock}
이 단순한 성질이 zonal-mean 운동량/열역학 방정식에서 \emph{longitudinal divergence 항이 사라지는 이유}이다.
예컨대 \eqref{eq:ch03_hw4_divH}의 $\partial_{\lambda} A_{\lambda}$ 항은 zonal mean을 취하는 순간
\eqref{eq:ch03_hw4_zonalvanish}에 의해 정확히 $0$이 되어, zonal-mean 식에는 오직
$\partial_{\phi}(\cdot\cos\phi)$ 형태의 meridional flux divergence와 연직 항만 남는다.
이는 본 챕터 \S\ref{sec:ch03-eddy-decomp} 이후의 모든 zonal-mean diagnostics ---
\eqref{eq:vE2-structure}의 $\overline{v_{E}^{2}}$ 구조에서부터
\S4의 zonal-mean precipitation budget까지 --- 의 토대가 된다.
\end{hisublock}
...
\begin{figure}[ht]
  ...
  \includegraphics[width=0.7\textwidth]{images/ch03/p08_img01.jpeg}
...
\begin{figure}[ht]
  ...
  \includegraphics[width=0.4\textwidth]{images/ch03/p09_img04.png}
  \caption{Annual-mean $[\bar{v}]$ (m\,s$^{-1}$), latitude--pressure cross-section.  Note the much smaller contour interval
...
\begin{figure}[ht]
  ...
  \includegraphics[width=0.45\textwidth]{images/ch03/p10_img01.png}\hfill
  \includegraphics[width=0.45\textwidth]{images/ch03/p10_img02.png}
  \caption{Annual-mean geopotential height at 200\,hPa: (left)~full field $\bar{Z}$, (right)~eddy field $Z^{*}$. \hisu{NH $Z^{*}$: wave-2$\sim$3 패턴 (북미 동부, 동아시아 상공 기압골); SH: eddy 진폭이 훨씬 약함 --- 지형 강제 부재 반영}}
  \label{fig:ch03-z200-eddy}
\end{figure}
...
\end{equation}
여기서 $R$은 dry air의 gas constant이고 $f$는 Coriolis parameter이다 (Holton 12판 식 (3.30)).
NH에서 meridional temperature gradient $\partial T/\partial y < 0$ (온도가 극 방향으로 감소)이므로
우변이 음의 값을 가지며, $\partial \ln p$ 또한 고도 증가 시 음이므로
$\partial u_g/\partial z > 0$, 즉 westerly wind는 고도에 따라 \emph{증가}하여
$\sim$200\,hPa에서 subtropical jet을 형성한다.
...
\begin{figure}[ht]
  ...
  \includegraphics[width=0.4\textwidth]{images/ch03/p17_img02.png}\hfill
  \includegraphics[width=0.4\textwidth]{images/ch03/p17_img01.png}
  \caption{Annual zonal-mean (left) $[\bar{T}]$ (K) and (right) $[\bar{u}]$ (m\,s$^{-1}$),
...
\[
  \Delta u \approx -\frac{R}{f}\,\frac{\Delta T}{\Delta y}\,\Delta \ln p
           ...
           \approx 39\;\text{m\,s}^{-1}.
\]
($\Delta T = -15$\,K (poleward 감소), $\Delta \ln p = \ln(200/850) \approx -1.45$로
부호를 모두 대입하면 $\Delta u$가 양수가 되어 상층 서풍 강화로 귀결된다.)
...
\begin{figure}[ht]
  ...
  \includegraphics[width=0.4\textwidth]{images/ch03/p16_img01.png}\hfill
  \includegraphics[width=0.4\textwidth]{images/ch03/p16_img02.png}
  \caption{Annual zonal-mean (left) $[\bar{\omega}]$ (Pa\,s$^{-1}$) and
           (right) $[\bar{v}]$ (m\,s$^{-1}$), showing the Hadley cell structure. \hisu{$\omega<0$ (상승)은 적도 중심, $\omega>0$ (하강)은 아열대; $[\bar{v}]$: 하층 적도 수렴, 상층 극방향 발산}}
...
\begin{figure}[ht]
  ...
  \includegraphics[width=0.55\textwidth]{images/ch03/p14_img01.jpeg}
...
\begin{remark}[Stationary Wave Amplitude --- Winter vs.\ Summer]
...
\begin{figure}[ht]
  ...
  \includegraphics[width=0.45\textwidth]{images/ch03/p19_img01.png}\hfill
  \includegraphics[width=0.45\textwidth]{images/ch03/p19_img02.png}
  \caption{Sea-level pressure (hPa) for (left)~DJF and (right)~JJA. \hisu{DJF: 깊은 Aleutian/Icelandic Low, 강한 시베리아 고기압 ($\sim$1040\,hPa); JJA: 대륙 열적 저기압 (아시아, 북미), 강한 해양 아열대 고기압}}
...
\begin{figure}[ht]
  ...
  \includegraphics[width=0.45\textwidth]{images/ch03/p21_img01.png}\hfill
  \includegraphics[width=0.45\textwidth]{images/ch03/p21_img04.png}
  \caption{850\,hPa wind (m\,s$^{-1}$) for (left)~DJF and (right)~JJA. \hisu{JJA: 남아시아 몬순의 핵심 --- 적도 횡단 소말리 제트가 동아프리카 해안을 따라 인도 아대륙으로 수분 유입}}
...
\begin{figure}[ht]
  ...
  \includegraphics[width=0.45\textwidth]{images/ch03/p22_img02.jpeg}\hfill
  \includegraphics[width=0.45\textwidth]{images/ch03/p22_img01.jpeg}
  \caption{200\,hPa wind (m\,s$^{-1}$) for (left)~DJF and (right)~JJA. \hisu{DJF: NH 아열대 제트 최강, 동아시아 상공 $>$50\,m\,s$^{-1}$; JJA: NH 제트 약화, SH 제트 강화}}
...
\begin{figure}[ht]
  ...
  \includegraphics[width=0.4\textwidth]{images/ch03/p23_img01.png}\hfill
  \includegraphics[width=0.4\textwidth]{images/ch03/p23_img02.png}
  \caption{Zonal-mean zonal wind $[\bar{u}]$ (m\,s$^{-1}$) for (left)~DJF and (right)~JJA. \hisu{DJF: NH 제트 강화 ($\sim$28\,m\,s$^{-1}$, 적도 쪽 이동); JJA: SH 제트 강화, NH 제트 약화 및 극 쪽 이동 --- 겨울 반구 온도 경도가 최대}}
...
\begin{figure}[ht]
  ...
  \includegraphics[width=0.4\textwidth]{images/ch03/p24_img01.png}\hfill
  \includegraphics[width=0.4\textwidth]{images/ch03/p24_img04.png}
  \caption{Zonal-mean meridional wind $[\bar{v}]$ (m\,s$^{-1}$) for (left)~DJF and (right)~JJA,
...
\begin{figure}[ht]
  ...
  \includegraphics[width=0.4\textwidth]{images/ch03/p25_img01.png}\hfill
  \includegraphics[width=0.4\textwidth]{images/ch03/p25_img04.png}
  \caption{Zonal-mean temperature $[\bar{T}]$ (K) for (left)~DJF and (right)~JJA. \hisu{가장 급한 남북 $T$ 경도는 항상 겨울 반구; 열대 대류권계면: $\sim$100\,hPa, $\sim$190\,K (높고 차가움); 중위도 대류권계면: $\sim$300\,hPa, $\sim$220\,K (낮고 따뜻함)}}
...
이 zonal-mean 온도장이 시간에 따라 어떻게 진화하는지를 진단하려면, 압력좌표 위에서
열역학 제1법칙을 \emph{flux form}으로 다시 쓰는 작업이 필요하다.
다음 두 example은 그 과정을 단계별로 시연한다.

\begin{example}[열역학 방정식의 flux form 변환]
\label{ex:ch03_hw4_fluxform}
압력좌표 위의 열역학 제1법칙
\begin{equation}
  c_{p}\,\frac{dT}{dt} \;-\; \alpha\,\omega \;=\; H,
  \label{eq:ch03_hw4_thermo1}
\end{equation}
에서 출발하자 ($\alpha = RT/p$는 specific volume, $H$는 단위질량당 가열률, $\omega = dp/dt$).
$dT/dt = \partial_{t}T + \vec v\cdot\nabla T + \omega\,\partial_{p}T$ 전개와 이상기체 상태방정식을 대입하면
\begin{equation}
  \frac{\partial T}{\partial t} \;+\; \vec v\cdot\nabla T \;+\; \omega\,\frac{\partial T}{\partial p}
       \;-\; \frac{R}{c_{p}\,p}\,\omega\,T \;=\; \frac{1}{c_{p}}\,H.
  \label{eq:ch03_hw4_thermo2}
\end{equation}
\textgr{Flux form}을 얻기 위해 압력좌표 연속방정식
\begin{equation}
  \nabla\!\cdot\vec v \;+\; \frac{\partial \omega}{\partial p} \;=\; 0
  \label{eq:ch03_hw4_cont}
\end{equation}
의 양변에 $T$를 곱한 식 $T(\nabla\!\cdot\vec v) + T\,\partial_{p}\omega = 0$을
\eqref{eq:ch03_hw4_thermo2}에 더한다.  이때
$T\,\nabla\!\cdot\vec v + \vec v\cdot\nabla T = \nabla\!\cdot(\vec v T)$,
$T\,\partial_{p}\omega + \omega\,\partial_{p}T = \partial_{p}(\omega T)$로 결합되므로
\begin{equation}
\boxed{\;
  \frac{\partial T}{\partial t}
     \;+\; \nabla\!\cdot\!\bigl(\vec v\,T\bigr)
     \;+\; \frac{\partial}{\partial p}\bigl(\omega\,T\bigr)
     \;-\; \frac{R}{c_{p}\,p}\,\omega\,T
     \;=\; \frac{1}{c_{p}}\,H.
\;}
  \label{eq:ch03_hw4_fluxform}
\end{equation}
각 단계의 물리적 정당화: (i) 연속방정식 곱셈은 $T$의 추가 source/sink를 만들지 않고
``질량보존을 만족하는 흐름에서는 advective와 flux 형태가 서로 변환 가능''함을 이용한 것이다.
(ii) 좌변 둘째--셋째 항은 \emph{온도(=enthalpy/$c_{p}$)의 3차원 flux divergence}로 묶인다.
(iii) $-(R/c_{p}p)\omega T$ 항은 단열 압축/팽창에 의한 온도 변화 ($\omega > 0$에서 압축 가열)를 담당한다.
\end{example}

\begin{example}[Zonal-mean 및 vertical-mean 온도 방정식]
\label{ex:ch03_hw4_zvT}
\eqref{eq:ch03_hw4_fluxform}에 zonal mean $\langle\cdot\rangle_{Z} = \frac{1}{2\pi}\!\int_{0}^{2\pi}\!d\lambda$를 적용한다.
\eqref{eq:ch03_hw4_divH}로 $\nabla\!\cdot(\vec v T)$를 풀어쓰면 longitudinal 항은
proposition~\ref{prop:ch03_hw4_zonalvanish}에 의해 사라지고, 남는 meridional 항은
$\frac{1}{a\cos\phi}\partial_{\phi}(\langle vT\rangle_{Z}\cos\phi)$ 형태가 된다.  따라서
\begin{equation}
  \frac{\partial T_{Z}}{\partial t}
     \;+\; \frac{1}{a\cos\phi}\,\frac{\partial}{\partial\phi}\!\bigl(\langle vT\rangle_{Z}\cos\phi\bigr)
     \;+\; \frac{\partial}{\partial p}\,\langle\omega T\rangle_{Z}
     \;-\; \frac{R}{c_{p}\,p}\,\langle\omega T\rangle_{Z}
     \;=\; \frac{1}{c_{p}}\,\langle H\rangle_{Z},
  \label{eq:ch03_hw4_zvT_main}
\end{equation}
여기서 $T_{Z} \equiv \langle T\rangle_{Z}$.  이어서 연직 평균
$\langle\cdot\rangle_{M} \equiv \frac{1}{p_{0}}\!\int_{0}^{p_{0}}\!\cdot\;dp$를 취하면,
flux divergence 항 $\int_{0}^{p_{0}}\partial_{p}(\langle\omega T\rangle_{Z})\,dp
= \langle\omega T\rangle_{Z}\!\bigl|_{p_{0}} - \langle\omega T\rangle_{Z}\!\bigl|_{0}$가
경계조건 $\omega(p=0) = \omega(p=p_{0}) = 0$에 의해 정확히 \emph{0}이 된다.  결과적으로
\begin{equation}
  \frac{\partial\,\langle T\rangle_{ZM}}{\partial t}
     \;+\; \frac{1}{a\cos\phi}\,\frac{\partial}{\partial\phi}\!\Bigl(\langle vT\rangle_{ZM}\cos\phi\Bigr)
     \;-\; \biggl\langle\frac{R}{c_{p}p}\,\omega T\biggr\rangle_{\!ZM}
     \;=\; \frac{1}{c_{p}}\,\langle H\rangle_{ZM},
  \label{eq:ch03_hw4_zvT_final}
\end{equation}
이 되어, 자오면 수송 $\langle vT\rangle_{ZM}$과 잔여 단열 항만이 평균 가열 $\langle H\rangle_{ZM}$과 균형을 이룬다.
\end{example}

\begin{hisublock}
\textbf{$\langle\omega T\rangle_{ZM}$의 구조.}
\eqref{eq:ch03_hw4_zvT_final}의 단열 항 $-\langle R\omega T/(c_{p}p)\rangle_{ZM}$은 지역적으로는
\emph{작지 않다}.  글로벌 평균에서 $\langle\omega\rangle = 0$이므로 $\langle\omega\bar T\rangle = 0$이지만,
$\omega T = \bar\omega\,\bar T + \omega' T'$로 분해하면 \emph{eddy 부분} $\langle\omega' T'\rangle$가
열대 (Hadley cell upper branch)에서 매우 크다 --- 따뜻한 공기 상승 $\Rightarrow \omega' T' > 0$,
즉 baroclinic conversion (APE $\to$ KE)의 주요 항.  중위도 자유대기에서는 이 항이 작은 반면,
글로벌·연평균에서는 ITCZ의 강한 $\omega'T' > 0$와 아열대 강하의 $\omega'T' < 0$이 부분적으로
상쇄된다.  Hadley/Ferrel cell의 평균 자오 순환은 $\bar v \sim 0.3$\,m\,s$^{-1}$로 약하지만,
\emph{eddy 단열 항} $\omega' T'$는 자오 eddy heat flux $\langle vT\rangle_{ZM}$와 함께
열대 가열 균형의 핵심이다 (Ch.~4 \S2 참조).
\end{hisublock}
...
\begin{figure}[ht]
  ...
  \caption{Quasi-biennial oscillation (QBO) in equatorial ($10^{\circ}$S--$10^{\circ}$N) stratospheric temperature anomalies $[T]$, 2002--2014. Each trace shows the time series at a fixed altitude (12--24\,km, offset vertically); the 3\,K reference bar at left sets the amplitude scale. \hisu{$\sim$16--24\,km 사이에서 동풍/서풍 레짐에 대응하는 온도 진동이 뚜렷; 주기 $\sim$26개월, 진폭 $\sim$3\,K; 14\,km 이하에서는 신호 약함}}
  \label{fig:ch03-qbo}
\end{figure}
...
\begin{figure}[ht]
  ...
  \includegraphics[width=0.7\textwidth]{images/ch03/p29_img01.jpeg}
...
\begin{figure}[ht]
  ...
  \includegraphics[width=0.7\textwidth]{images/ch03/p30_img01.jpeg}
...
\begin{remark}[Major Precipitation Features]
\begin{itemize}
...
\begin{figure}[ht]
  ...
  \includegraphics[width=0.45\textwidth]{images/ch03/p32_img01.png}\hfill
  \includegraphics[width=0.45\textwidth]{images/ch03/p32_img02.png}
  \caption{Annual-mean (left) precipitation (mm\,day$^{-1}$) and (right) 500\,hPa vertical velocity $\bar{\omega}$ (Pa\,s$^{-1}$), showing the spatial co-location of ascent ($\omega<0$) and heavy rainfall. \hisu{상승 운동($\omega<0$) 지역 = 다우 지역; 강한 공간 상관 --- 대규모 상승이 단열 냉각, 포화, 응결, 강수를 유발}}
...
\begin{figure}[ht]
  ...
  \includegraphics[width=0.7\textwidth]{images/ch03/p33_img01.png}
...
\begin{figure}[ht]
  ...
  %   dominates).
  \includegraphics[width=0.7\textwidth]{images/ch03/p35_img01.jpeg}
...
\begin{figure}[ht]
  ...
  \includegraphics[width=0.7\textwidth]{images/ch03/p36_img01.jpeg}
...
\begin{figure}[ht]
  ...
  %   Pacific.
  \includegraphics[width=0.45\textwidth]{images/ch03/p36_img01.jpeg}\hfill
  \includegraphics[width=0.45\textwidth]{images/ch03/p38_img01.png}
  \caption{Annual-mean (left) SST ($^{\circ}$C) and (right) precipitation (mm\,day$^{-1}$).
           Heavy rainfall occurs over the warmest SSTs, but convergence is also required. \hisu{고SST만으로는 불충분 --- 대규모 하층 수렴이 필요; 동적도태평양: SST $>$$26^{\circ}$C이지만 Walker 순환 하강으로 강수 적음}}
...
\begin{figure}[ht]
  ...
  %   distribution.
  \includegraphics[width=0.45\textwidth]{images/ch03/p36_img01.jpeg}\hfill
  \includegraphics[width=0.45\textwidth]{images/ch03/p39_img02.png}
  \caption{Annual-mean (left) SST ($^{\circ}$C) and (right) 850\,hPa wind (m\,s$^{-1}$), illustrating the wind--SST coupling. \hisu{무역풍 $\rightarrow$ 적도 Ekman 발산/용승 $\rightarrow$ cold tongue 유지; 따뜻한 SST $\rightarrow$ 경계층 안정도 감소 $\rightarrow$ 지표 바람 강화 (양방향 결합)}}
...
\begin{figure}[ht]
  ...
  \includegraphics[width=0.7\textwidth]{images/ch03/hw3_skin_temp.png}
