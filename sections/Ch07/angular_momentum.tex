...
\begin{itemize}
  ...
  \item 따라서 단위 질량으로 보면 $M_\Omega \gg M_r$이며, 후술하듯 전체 질량(대기 + 고체 지구)을
        고려하면 그 차이는 $\sim 100$배까지 벌어진다.
\end{itemize}
\end{hisublock}

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.65\textwidth,height=0.5\textheight,keepaspectratio]{images/ch07/p05_img01.png}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.65\textwidth,height=0.5\textheight,keepaspectratio]{images/ch07/p13_img01.png}
...
\[
  ...
  \qquad R \simeq a,
\]
와 그 두 성분의 물리적 의미를 정리했다 ($R$은 지구 반지름;
회전축까지의 수직 거리는 $r_\perp = R\cos\phi$).  이 절에서는 \emph{개별 공기 덩어리}의
...
$M_\Omega = \Omega R^2\cos^2\phi$ (여기서 $R \simeq a$는 지구 반지름,
$r_\perp = R\cos\phi$는 회전축까지의 수직 거리)로 정의되어 있는데,
실제로는 $r_\perp = R\cos\phi$가 위도와 함께
...
\begin{equation}\label{eq:ch07_cons_fluxform}
  ...
  \begin{aligned}
    ...
    &-\,a\cos\phi\!\left[\,
        ...
        \;+\;
        g\,\frac{\partial \tau_E^x}{\partial \sigma}
      \,\right].
  \end{aligned}
  \;}
\end{equation}
...
\begin{itemize}
  ...
  \item $-a\cos\phi\, RT\,\partial_x p_s$: $\sigma$-좌표로 옮길 때 생긴
        ...
        결국 산악 토크로 \emph{합쳐진다}.
  \item $-g\,a\cos\phi\,\partial_\sigma \tau_E^x$: 연직 eddy stress
...
\end{align}
즉 두 항이 깔끔히 \emph{$p_s\,\partial_x(\Phi - RT)$} (이후 step 3에서
$\sigma$-연직 미분 형태로 변환)와 \emph{$\partial_x(p_s RT)$} (zonal 평균
시 주기성으로 0)로 갈라진다.  핵심은 다음 step 3의 항등식이다.
...
\eqref{eq:ch07_cons_step2_raw}에서 \emph{$\partial/\partial x$가 바깥에 있는
total derivative} 항 $\partial_x(p_s RT)$는 $\partial_x =
(a\cos\phi)^{-1}\partial_\lambda$이므로
proposition~\ref{prop:ch07_cons_zonal_vanish}에 의해 zonal mean을 취하면
$\bigl(\partial_x(p_s RT)\bigr)_Z = 0$이다.  주의: 일반적으로
$(p_s\,\partial_x RT)_Z \neq 0$이고 $(RT\,\partial_x p_s)_Z \neq 0$이지만
($p_s$와 $RT$가 곱해진 형태라 그 product가 \emph{total} zonal derivative일
때만 0), 두 항의 \emph{합}인 $\partial_x(p_s RT)$만이 total derivative이므로
주기성에 의해 0이 된다.  따라서 남는 것은
\begin{equation}\label{eq:ch07_cons_step4_pgrad}
  ...
  + \underbrace{\left(\frac{\partial(p_s RT)}{\partial x}\right)_{\!Z}}_{=0 \text{ (periodic)}},
\end{equation}
...
정역학 평형 $dp = -\rho g\,dz$ (상향 $z$) 하에서, 단위 수평면적당
질량 요소는
\begin{equation}\label{eq:ch07_cons_mass}
  \rho\,dz \;=\; -\,\frac{1}{g}\,dp \;=\; -\,\frac{p_s}{g}\,d\sigma
  \qquad\bigl(z\uparrow,\;\sigma:0\to 1\text{ 하향}\bigr),
\end{equation}
이고, 부호 반전이 적분 방향($\sigma: 0\to 1$이 $z$ 감소 방향)에서 흡수되어
column mass element는
\[
  \frac{dm}{A} \;=\; \frac{p_s}{g}\,d\sigma,
\]
로 쓰며, 단위 수평면적 위 전체 대기 기둥의 질량은 $\int_0^1 (p_s/g)\,d\sigma
= p_s/g$이다.
\end{definition}
...
취한 \emph{mass-integrated} 절대 각운동량 수지 (단위 위도 길이당, 단위
kg\,m$^2$\,s$^{-2}$ = N\,m)는 정확히 세 항으로 닫힌다.
\begin{equation}\label{eq:ch07_cons_threeterm}
  \boxed{\;
  \frac{\partial}{\partial t}\!\!\int_0^1\!\!\frac{(p_s M)_Z}{g}\,d\sigma
  \;=\;
  \underbrace{-\,\frac{1}{\cos\phi}\frac{\partial}{\partial y}\!\!\int_0^1\!
              \frac{(p_s M v)_Z}{g}\cos\phi\,d\sigma}_{\text{(I) Meridional flux convergence}}
  \;\underbrace{-\,a\cos\phi\,(\tau_E^x)_Z\big|_{\sigma=1}}_{\text{(II) Friction torque}}
  \;\underbrace{-\,a\cos\phi\,\bigl(p_s\,\partial_x h\bigr)_Z}_{\text{(III) Mountain torque}}.
  \;}
\end{equation}
($1/g$ 인자는 column mass element $dm/A = p_s d\sigma/g$ 정의에서
자연스럽게 따라 나온다; 이 정의로 LHS와 flux는 \emph{mass-integrated} AM
[kg\,m$^2$\,s$^{-1}$], 우변 torque 항들은 [N\,m\,m$^{-1}$]로 일관된 단위를
갖는다.)
\end{theorem}
...
\begin{enumerate}
  ...
  \item \textbf{(III) Mountain torque} -- 산악 동·서면 기압차에 의한 토크.
        부호 분석: 서쪽 high, 동쪽 low 패턴이면 $(p_s\,\partial_x h)_Z > 0$
        (지표압이 큰 곳에서 $h$가 증가, 즉 서쪽 산비탈이 풍상측),
        따라서 $T_M = -a\cos\phi(p_s\,\partial_x h)_Z < 0$ → 대기가 지표(고체
        지구)에 westerly AM을 \emph{넘긴다} (= 대기는 AM을 \emph{잃는다}).
        반대로 $-(p_s\,\partial_x h)_Z > 0$이면 대기가 지표에서 westerly AM을
        \emph{받는다}.  현실 NH 중위도 평균에서 대기→지구 AM 교환의
        \emph{약 50\%}를 이 항이 담당한다.
\end{enumerate}
\smallskip
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.65\textwidth,height=0.5\textheight,keepaspectratio]{images/ch07/p27_img01.png}
...
따라서 SH에서는 거의 \textit{전적으로 frictional torque}만이 대기-지구 AM
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.65\textwidth,height=0.5\textheight,keepaspectratio]{images/ch07/p29_img01.png}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.65\textwidth,height=0.5\textheight,keepaspectratio]{images/ch07/p30_img01.png}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.65\textwidth,height=0.5\textheight,keepaspectratio]{images/ch07/p31_img01.png}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.65\textwidth,height=0.5\textheight,keepaspectratio]{images/ch07/p33_img01.png}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.65\textwidth,height=0.5\textheight,keepaspectratio]{images/ch07/p35_img01.png}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.65\textwidth,height=0.5\textheight,keepaspectratio]{images/ch07/p36_img01.png}
...
파헤친다.  AM 자체의 정의 $M = \Omega a^2\cos^2\phi + u\,a\cos\phi
= M_\Omega + M_r$에서, 상대운동 성분 $u$를 zonal-mean / eddy로 더 분해한다
($u = u_Z + u_E$, 단 $(u_E)_Z = 0$).  Solid-rotation 성분 $\Omega a^2\cos^2\phi$는
경도에 의존하지 않으므로 이미 $u_Z$와 같은 zonal-mean side에 속한다.  결국
\begin{align}
  M &= \underbrace{\Omega a^2\cos^2\phi + u_Z\,a\cos\phi}_{M_Z}
       + \underbrace{u_E\,a\cos\phi}_{M_E},
...
\begin{equation}\label{eq:ch07_trans_three_flux}
  (p_s M v)_Z
  \;=\;
  \underbrace{\Omega a^2\cos^2\phi\,(p_s v)_Z}_{\text{solid-rotation flux}}
  \;+\;
...
\begin{enumerate}
  \item \textbf{Solid-rotation flux} $\Omega a^2\cos^2\phi\,(p_s v)_Z$:
...
\begin{equation}\label{eq:ch07_trans_solid_zero}
  \int_0^1 \Omega a^2\cos^2\phi\,(p_s v)_Z\,d\sigma
  \;=\;
  \Omega a^2\cos^2\phi \int_0^1 (p_s v)_Z\,d\sigma
...
그 수송 channel을 막아버리므로, $\Omega a^2\cos^2\phi$ 항은 자동적으로 사라진다.
...
\begin{equation}\label{eq:ch07_trans_eddy_approx}
  \int_0^1 (p_s M v)_Z\,\cos\phi\,d\sigma
  \;\approx\;
  \int_0^1 a\,p_{s,Z}\,(u_E v_E)_Z\,\cos^2\phi\,d\sigma.
\end{equation}
여기서 우리는 (i) solid-rotation 항이 mass 보존으로 사라지고
(\eqref{eq:ch07_trans_solid_zero}), (ii) mid/high-latitudes에서는 drift 항
($a\cos\phi\,u_Z(p_s v)_Z$)이 eddy 항에 비해 작으며 (Ferrel cell의 $(p_s v)_Z$
약함), (iii) $p_s$의 변동률이 $v_E$의 변동률보다 훨씬 작다는 ($\delta p_s/p_s
\ll \delta v_E / v_E$) 근사를 사용했다.  RHS의 $\cos^2\phi$ 인자는 eddy
flux의 $a\cos\phi$ geometry와 LHS의 추가 $\cos\phi$ (자오 line element)에서
유래한다.

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.65\textwidth,height=0.5\textheight,keepaspectratio]{images/ch07/p42_img01.png}
...
% [tilt schematic merged into the QG argument figure below; the SW--NE
%  trough tilt is illustrated as part of Figure~\ref{fig:ch07_trans_qg_argument}.]
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.65\textwidth,height=0.5\textheight,keepaspectratio]{images/ch07/p44_img01.png}
...
사실은 관측에서 확인된다 (Figure~\ref{fig:ch07_trans_qg_argument}).
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.65\textwidth,height=0.5\textheight,keepaspectratio]{images/ch07/p47_img01.png}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.65\textwidth,height=0.5\textheight,keepaspectratio]{images/ch07/p49_img01.png}
  \caption{Four-panel decomposition of the vertically integrated zonal-mean
  meridional angular momentum flux $[\overline{v\,M}]$ across latitude, shown
  for DJF, JJA, and annual (YEAR) means.  Panel \textbf{(a)} mean meridional
  advection of zonal-mean AAM $[\bar v\,\bar M]$ (Hadley/Ferrel cell branch);
  \textbf{(b)} transient eddy flux $\overline{v'M'}$;
  \textbf{(c)} stationary eddy flux $[\bar v^{*}\bar M^{*}]$ (zonal departure);
  \textbf{(d)} total $[\overline{v\,M}]$ — the sum of (a)+(b)+(c).
  Outside $\pm 10^\circ$ of the equator, (b) the transient eddy term
  dominates; only in the deep tropics does (a) the mean meridional
  circulation contribute comparably.
  \hisu{자오 AAM flux의 4-panel 분해.  (a) 평균 자오순환 $[\bar v\bar M]$,
  (b) transient eddy $\overline{v'M'}$, (c) stationary eddy
  $[\bar v^{*}\bar M^{*}]$, (d) 총합.  적도 $\pm 10^\circ$ 밖에서는 (b)
  transient eddy가 거의 모든 flux를 담당.}}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.65\textwidth,height=0.5\textheight,keepaspectratio]{images/ch07/p51_img01.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.65\textwidth,height=0.5\textheight,keepaspectratio]{images/ch07/p53_img01.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.65\textwidth,height=0.5\textheight,keepaspectratio]{images/ch07/p55_img01.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.65\textwidth,height=0.5\textheight,keepaspectratio]{images/ch07/p56_img01.png}
...
적도 방향 ($v_Z < 0$ in NH lower branch)으로 흐르므로, zonal 운동방정식에서
Coriolis 가속도는 (부호 규약: $\partial_t u = +fv + \cdots$)
...
\noindent
즉 \emph{easterly (westward)} 방향이다.  그러나 trade wind는 시간에 따라 거의
변하지 않으므로 ($\partial u_Z/\partial t \approx 0$), 이 easterly tendency를
상쇄하기 위해 무언가가 \emph{westerly}를 공급해야 한다.  그 무언가가 바로
\textgr{surface friction}이다 (지표 동풍에 작용하는 drag는 음의 $u$를
0으로 끌어올리는 방향, 즉 westerly tendency로 작용).

\begin{equation}\label{eq:ch07_temp_pointA_bal}
  \boxed{
    +f\,v_Z - \frac{1}{\rho}\frac{\partial \tau^x}{\partial z} = 0
...
유지된다.  적도 지표에서 Coriolis $+fv_Z < 0$ (Hadley 하층 $v_Z<0$이라
westward)이고, friction이 같은 크기의 westerly로 받쳐준다.  이 과정에서
atmosphere는 earth로부터 westerly AM을 \emph{얻고}, 같은 양을 mid-latitude로
흘려보내는 데 사용한다.}
...
\noindent
즉 \emph{westerly} (eastward) 방향으로 가속하려 한다.  지표 westerly에 대해
friction은 brake이므로 (서풍을 동풍 방향으로 깎음), Ferrel cell Coriolis
공급과 friction loss가 직접 균형을 이룬다고 생각할 수도 있다.  그러나
정량적으로 살펴보면 Ferrel cell의 $|v_Z|$가 작아서 Coriolis 공급만으로는
충분치 않다.  실제 균형은 \textgr{eddy momentum flux convergence}가
주요 westerly source로 들어와야 완성된다.
...
\noindent
즉 Coriolis는 \emph{westerly} (eastward)로 가속한다.  이것이 Hadley cell
상층이 subtropical jet에 westerly AM을 공급하는 \emph{Coriolis 측면의 메커니즘}이다.
AAM 보존 관점에서 동일한 사실을 다시 표현하면: 적도에서 출발한 공기는 큰
$M_\Omega$를 가지고 있고, 그것을 ($u, \phi$가 변하면서) $M = M_\Omega + M_r$로
보존한다.  $\phi$가 증가하면 $\cos\phi$가 감소하고, $M_\Omega = \Omega
R^2\cos^2\phi$가 줄어든다.  보존되려면 $M_r = u R\cos\phi$가 그만큼 늘어야
하므로 $u$가 증가한다.  바로 이것이 \emph{Hadley cell 상층 가지가 westerly를
가져다 주는} 이유다.

\emph{고립된 (eddy-free)} Hadley cell 모형에서는 이 Coriolis-driven westerly
공급이 advection과 함께 angular-momentum conserving wind를 만든다 (Held \&
Hou, 1980).  그러나 실제 대기에서는 상층 westerly가 angular-momentum
conserving 값보다 훨씬 \emph{작다}.  그 이유는 \textbf{eddy momentum flux
divergence}가 westerly를 가져가버리기 때문이다.  따라서 B점의 정상 상태
균형은 Hadley cell의 westerly 공급(Coriolis $+fv > 0$)과 eddy divergence의
westerly 손실이 같아질 때 이루어진다.

\begin{equation}\label{eq:ch07_temp_pointB_bal}
  \boxed{
    \underbrace{+f\,v_Z}_{\text{Coriolis (west)}}
...
부근이다.  이는 \emph{우연이 아니다}.  단순 AM 보존 ($u(\phi) = \Omega R
\sin^2\phi/\cos\phi$)만 보면 $u$가 발산하는 지점은 \emph{극} ($\cos\phi \to 0$)
이지 30$^\circ$가 아니다.  Hadley cell 끝이 약 30$^\circ$에 위치하는
까닭은 단일 효과가 아니라 \emph{여러 제약의 조합}이다:
\begin{itemize}
  \item \textbf{AM 보존} (Held \& Hou, 1980): 적도 부근 가열에 의한 상층
    공기 이동이 AM 보존 풍속을 발생시키지만, 30$^\circ$ 근방에서 $u$가 너무
    커져 thermal wind balance 및 정역학과 양립 가능한 단열 평형해를 더는
    유지할 수 없다.
  \item \textbf{Thermal wind / energy 제약}: Held--Hou 이론은
    \emph{moist static energy의 수평 균등화 + thermal wind balance + AM
    conservation}을 동시에 만족하는 닫힌 axisymmetric 해를 풀어 cell width
    $\phi_H \sim (gH\Delta_h/\Omega^2 a^2)^{1/2}$를 얻는다.  지구 매개변수에
    대입하면 $\phi_H \approx 25$--$30^\circ$.
  \item \textbf{Baroclinic instability}: 같은 위도에서 baroclinic eddy가
    강하게 일어나 eddy가 잔여 westerly를 더 극쪽으로 운반한다 (Lindzen \&
    Hou, 1988).
\end{itemize}
지표 trade-wind belt와 mid-latitude westerly belt의 경계가 동일한 위도에 있는
...
\begin{enumerate}
  \item \textbf{AM의 분해}: $M = \Omega R^2\cos^2\phi + uR\cos\phi
    ...
    분리되며, 단위질량당 비율 $M_\Omega/M_r$은 위도에 따라 0--46.  대기 전체
    질량 적분 기준으로는 $M_\Omega/M_r \sim 100$ 정도이다.
  \item \textbf{Conservation-only argument의 비현실성}: 적도 공기를 30$^\circ$로
    이동시키면 $u\approx 134$ m/s, 60$^\circ$로 이동시키면 $u \approx 696$ m/s
    ($u(\phi) = \Omega R\sin^2\phi/\cos\phi$)라는 비현실적 값이 나온다.
    \emph{Eddy transport}가 반드시 필요하다.
...
\begin{itemize}
  ...
  \item Rosen \& Salstein (1983, \textit{J.\ Geophys.\ Res.}, 88, 5451--5470)
    --- LOD-AAM correspondence의 원논문, monthly time series 분석 방법론
    정립.  Rosen, Salstein \& Wood (1990, \textit{J.\ Geophys.\ Res.}, 95,
    265--279, ``Discrepancies in the Earth--atmosphere angular momentum
    budget'') --- 본 책 Fig.~\ref{fig:ch07_temp_monthly_lod_aam} 관련 AAM
    budget closure를 정량적으로 검증한 후속 논문.
  ...
  \item Wallace, Battisti, Thompson, Hartmann (2023), \textit{The Atmospheric
    General Circulation}, Cambridge UP, Ch.~3 (Angular Momentum Balance) ---
    본 책 Ch.~7의 구조가 가장 가까운 modern reference.
...
Circulation}, Cambridge University Press, Ch.~3;
